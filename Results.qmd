...
::: {#fig-spectrograms layout-nrow=3}

![Anger](./anger.png){#fig-anger}

![Fear](./fear.png){#fig-fear}

![Happy](./happy.png){#fig-happy}

Spectograms of the Audio files for Anger, Fear and Happy
:::

Based on the spectrograms of the different audio samples of different emotions (see @fig-spectrograms), it is possible to infer certain characteristics of each emotion. For example, samples with positive emotions such as happiness and excitement tend to have higher frequency content, with more energy concentrated in the higher frequency ranges (see @fig-happy). In contrast, samples with negative emotions such as sadness and anger tend to have lower frequency content, with more energy concentrated in the lower frequency ranges (see @fig-anger). Additionally, emotions such as fear and surprise may exhibit sudden changes in the frequency content (see @fig-fear), whereas emotions such as boredom and neutrality may exhibit more stable and uniform frequency patterns. However, it is important to note that the interpretation of spectrograms is subjective and depends on the context and individual perception.

### B. DISTRIBUTION OF EMOTIONS

![Emotion Distribution](./emotion_distribution.png){#fig-emotiondistribution}
...
## V. DATA PREPARATION {#sec-datapreparation}
...
The model is trained for 370 epochs. The loss on the training and validation sets are plotted in @fig-lossviteration . The loss on the training set decreases with each epoch, while the loss on the validation set decreases until the 150th epoch and then becomes constant.

![Loss v/s Iterations](./loss_vs_iterations.png){#fig-lossviteration}
...
After training the model the accuracy is around 65%. The model is then tested on the test set that was created during data preparation (see @sec-datapreparation). There are a total of 25 audio samples that are in our test set and the model predictions of the first 10 samples are as follows:
